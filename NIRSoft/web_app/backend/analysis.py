...
import matplotlib
matplotlib.use('Agg')            # ← Burada GUI dışı backend'e geçiyoruz
...
def full_analysys(
    ...
    # 2) Geçici .snirf haline getir ve tekrar oku
    ...
    if len(trigger_indices) < 7:
        raise ValueError("En az 7 trigger bulunmalı.")

    # Segment aralıklarını belirle (Walk1: 0-1, Walk2: 2-3, Walk3: 4-5)
    ...
    # Yürüme segmentlerini uç uca ekle (Walk1, Walk2, Walk3)
    ...
    plt.plot(t, mean_hbo_time_all, color='black', linewidth=2)
    for idx in trigger_indices:
        plt.axvline(t[idx], color='red', linestyle='--', alpha=0.7)
...
###########################################################################################################################
    ...
    # # 13. sayfa: power (Z-score)
    # draw_logo()
    # img_path = os.path.join(save_dir, power_z_fn)
    # if os.path.exists(img_path):
    #     with Image.open(img_path) as im:
    #         ow, oh = im.size
    #     tw = w - 2*cm
    ...
    # # 14. sayfa: scatter_heatmap_fn ve scatter_heatmap_z_fn (aynı sayfa)
    # draw_logo()
    # img_path1 = os.path.join(save_dir, scatter_heatmap_fn)
    # img_path2 = os.path.join(save_dir, scatter_heatmap_z_fn)
    # y_top = h - 4*cm - 2*cm  # logonun altı
    # y_gap = 1*cm
    # img_height = (h - 7*cm - y_gap) / 2
    # tw = w - 2*cm
    ...
    #     th1 = min(th1, img_height)
    #     c.setFont("Helvetica-Bold", 13)
    #     # title1 = "Kanal Yerleşimi ve HbO Isı Haritası (fNIRS)"
    #     # tw_title1 = c.stringWidth(title1, "Helvetica-Bold", 13)
    #     # c.drawString((w-tw_title1)/2, y_top, title1)
    #     c.drawImage(img_path1, x=(w-tw)/2, y=y_top-th1-0.5*cm, width=tw, height=th1, preserveAspectRatio=True)
    ...
    #     y2 = y_top-th1-1.5*cm-th2
    #     c.setFont("Helvetica-Bold", 13)
    #     # title2 = "Kanal Yerleşimi ve HbO Isı Haritası (Z-score)"
    #     # tw_title2 = c.stringWidth(title2, "Helvetica-Bold", 13)
    #     # c.drawString((w-tw_title2)/2, y2+th2+0.2*cm, title2)
    #     c.drawImage(img_path2, x=(w-tw)/2, y=y2, width=tw, height=th2, preserveAspectRatio=True)
    # sayfa_numarasi()
